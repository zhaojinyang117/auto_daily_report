...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['all']:
            ...
            self.stdout.write(f"当前时间窗口({current_time.strftime('%H:%M')})内有{len(users_to_process)}个用户需要处理")
        ...
        for user in users_to_process:
            try:
                ...
                if result['success']:
                    success_count += 1
                    self.stdout.write(self.style.SUCCESS(f"用户[{user.username}]报告发送成功"))
                else:
                    fail_count += 1
                    self.stdout.write(self.style.WARNING(f"用户[{user.username}]报告发送失败: {result['message']}"))
            except Exception as e:
                fail_count += 1
                logger.exception(f"处理用户[{user.username}]报告时发生异常")
                self.stdout.write(self.style.ERROR(f"处理用户[{user.username}]时发生错误: {str(e)}"))
        
        # 输出汇总信息
        self.stdout.write("-" * 50)
        self.stdout.write(f"任务完成，成功: {success_count}，失败: {fail_count}，总计: {len(users_to_process)}")
        logger.info(f"日报发送任务完成，成功: {success_count}，失败: {fail_count}，总计: {len(users_to_process)}")
        ...
        try:
            ...
            try:
                ...
                if not settings.is_active:
                    self.stdout.write(self.style.WARNING(f"用户[{username}]的报告功能未激活"))
                    return
            ...
            if result['success']:
                ...
                self.stdout.write(self.style.ERROR(f"用户[{username}]的报告发送失败: {result['message']}"))
    ...
    def _get_users_for_current_time(self, current_time: time) -> List[User]:
        ...
        当前策略: 如果用户的设置时间与当前小时相同，则处理该用户的报告
